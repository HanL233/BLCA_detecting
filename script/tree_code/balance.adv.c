...
int compareFI2(const void *_a,const void *_b){
        FI *a=*(FI**)_a,*b=*(FI**)_b;
...
void sortFI2(FI **fi,int len){
        qsort(fi,len,sizeof(FI*),compareFI2);
}
...
int filterFI(FI ***fi,int sum,int least){
	float *GS=(float*)calloc(sum,sizeof(float));
	FI **F=*fi;
	sortFI2(F,sum);
	int i=0,j=0;
	for(i=0;i<sum;i++){
		...
		j=i-1;
		while(j>=0){
			float gs=exp((float)((F[i]->center-F[j]->center)*(F[i]->center-F[j]->center))/-10000.0f);
			GS[j]+=gs;
			if(gs<1e-4)break;
			j=j-1;
		}
	...
	float *_GS=(float*)malloc(sum*sizeof(float));
	memcpy(_GS,GS,sum*sizeof(float));
	qsort(_GS,sum,sizeof(float),compareF);
	j=(int)(((float)sum)*0.25f);
	if(j<least)j=sum-1>=least?least:sum-1;
	float s=_GS[j];
	j=0;
	for(i=0;i<sum;i++){
		if(GS[i]>=s)j++;
	}
	*fi=(FI**)malloc(j*sizeof(FI*));
	j=0;
	for(i=0;i<sum;i++){
		if(GS[i]>=s){
			(*fi)[j++]=F[i];
...
void getScore(float **ele,int x,int y,FI **fi,int len,char *table,char bit,int point,float *value){
        ...
        for(i=0;i<x;i++){
                ...
                for(j=0;j<point;j++){
                        ...
                        else score-=pValue2Score(fi[j]->pValue);//TODO new inserted
                }
                value[i]=score;
        }
}
...
int main(int argc,const char **argv){
        ...
        if(fl0 && fl1){
                ...
                fl0=filterFI(&fi0,fl0,max);
                fl1=filterFI(&fi1,fl1,max);
