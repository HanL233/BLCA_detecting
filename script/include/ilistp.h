...
list_p *createListP(){
	...
	p->head->p=(int*)malloc(sizeof(int));
	p->head->next=NULL;
	return p;
}

int pushListP(list_p **p,int val){
	if(*p==NULL)return 0;
	((*p)->head->p)[(*p)->here++]=val;
	(*p)->sum++;
	if((*p)->here>=(*p)->length){
		...
		tmp->next=(*p)->head;
		(*p)->length<<=1;
		tmp->p=(int*)malloc(((*p)->length)*sizeof(int));
		(*p)->here=0;
		(*p)->head=tmp;
	}
	return 1;
...
int pushListPP(list_p **p,const int *str){
	int sum=0,a=-1;
...
int getListArr(list_p *p,int **a){
	if(p==NULL)return 0;
	int *arr=(int*)malloc((p->sum+1)*sizeof(int));
...
int compareListP(list_p *p0,list_p *p1){
...
int fuseListP(list_p **p,list_p *src){
...
list_p *copyListP(list_p *p){
	...
	list_nod **b4=NULL;
	while(p->head){
		int *str=(int*)malloc(p->length*sizeof(int));
